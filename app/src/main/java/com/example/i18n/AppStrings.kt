...
object AppStrings {
    ...
    fun heroSlogan(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "The #1 Algerian Charades & Acting Party Game 🇩🇿🎭" else "لعبة التمثيل والشاراد الجزائرية الأولى لقعدات الصحاب والعائلة 🇩🇿🎭"
    fun wordsCountTitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "+270 Words" else "+270 كلمة"
    fun wordsCountSubtitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "100% DZ Cultural Bank" else "بنك جزائري أصيل"
    fun languageToggleTitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Language: EN" else "اللغة: عربي"
    fun languageToggleSubtitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Tap to switch to العربية" else "اضغط للتحويل إلى English"
    fun categoriesCapsuleTitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "8 Categories" else "8 فئات"
    fun categoriesCapsuleSubtitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Timer & settings" else "والتحكم بالعداد"
    ...
    fun participatingTeams(lang: AppLanguage, count: Int = 0) = if (lang == AppLanguage.ENGLISH) "Participating Teams ($count):" else "الفرق المشاركة ($count):"
    ...
    fun continueToReview(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Continue to Review 🚀" else "متابعة لمراجعة الماتش 🚀"
    fun sfxLabel(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Sound Effects & Audio" else "المؤثرات الصوتية"
    fun sfxSubtitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Buttons, buzzers, and victory tones" else "أصوات الأزرار وصفارة الوقت ونغمات الفوز"
    fun musicLabel(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Algerian Background Music" else "الموسيقى الجزائرية في الخلفية"
    fun musicSubtitle(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Chaabi, Rai, Gnawa, and modern tracks" else "طابع شعبي، راي، قناوي، وموسيقى حماسية"
    fun availableCategoriesLabel(lang: AppLanguage) = if (lang == AppLanguage.ENGLISH) "Available Categories" else "الفئات المتاحة"
    fun enabledOf(lang: AppLanguage, enabled: Int, total: Int) = if (lang == AppLanguage.ENGLISH) "($enabled of $total enabled)" else "($enabled من $total مفعلة)"
