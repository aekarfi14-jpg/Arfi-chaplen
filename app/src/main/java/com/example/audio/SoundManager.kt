...
class SoundManager(private val context: Context) {
    ...
    fun playTickWarning() {
        if (!sfxEnabled) return
        playSyntheticTone(987.77, 45, WaveType.SINE, 0.30f)
    }

    fun playMemeEventAlert() {
        if (!sfxEnabled) return
        audioScope.launch {
            playSyntheticTone(659.25, 70, WaveType.TRIANGLE, 0.35f)
            delay(50)
            playSyntheticTone(880.00, 120, WaveType.SINE, 0.40f)
        }
    }
